...
use sector_builder::{GetSealedSectorResult, PieceMetadata, SealStatus, SealTicket, SecondsSinceEpoch, StagedSectorMetadata, UnpaddedBytesAmount, SealedSectorMetadata};
...
pub unsafe extern "C" fn sector_builder_ffi_seal_staged_sector(
    ...
    match (*ptr).seal_staged_sector(
        ...
        *prover_id,
        ...
        Ok(meta) => {
            let sector = responses::FFISealedSectorMetadata::from(meta);
...
pub unsafe extern "C" fn into_sealed_sector_metadata(
    ...
    let p_aux = &(*sector_ptr).p_aux;
    let comm_c = from_raw_parts(p_aux.comm_c_ptr, p_aux.comm_c_len);
    let comm_r_last = from_raw_parts(p_aux.comm_r_last_ptr, p_aux.comm_r_last_len);
    let p_aux = PersistentAux {
        comm_c: PedersenDomain::try_from_bytes(comm_c).unwrap(),
        comm_r_last: PedersenDomain::try_from_bytes(comm_r_last).unwrap(),
    };
    ...
    SealedSectorMetadata {
        ...
        seal_ticket: SealTicket {
            block_height: (*sector_ptr).seal_ticket.block_height,
            ticket_bytes: (*sector_ptr).seal_ticket.ticket_bytes,
        },
    }
}
